...
function Page() {
    ...
    useEffect(() => {
        const getData = async () => {7
            const dataFromApi = await fetch(`https://newsapi.org/v2/top-headlines?country=us&category=business&apiKey=${import.meta.env.VITE_NEWS_API_KEY}`).then(res => res.json()).then(data => setTest(data?.articles))
        }
        getData()
        console.log(test)
    }, [])
  ...
  return (
    <main className="p-0">
        ...
        className='flex justify-between align-middle p-4 mb-4 w-[90%] mx-auto'>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 w-[90%] mx-auto pb-5">
            {test?.map((dat: { urlToImage: any; title: any; description: any; author: any; source: { name: any }; publishedAt: any }, index: any) => (
                <Card 
                key={index} 
